...
export function CloudCaseStudies() {
    const s = useStyles();
    return (
        <section className={s.section}>
            <div className={s.inner}>
                ...
                <div className={s.grid}>
                    {studies.map((study) => (
                        <a key={study.title} className={s.card} href={study.href} target="_blank" rel="noreferrer">
                            <span className={s.pill}>{study.tag}</span>
                            <h3 className={s.cardTitle}>{study.title}</h3>
