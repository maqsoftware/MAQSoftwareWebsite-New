...
const useStyles = makeStyles({
    ...
    title: { fontSize: "30px", fontWeight: 700, color: "var(--maq-black)", margin: "0 0 6px", letterSpacing: "-0.01em" },
    ...
    detailName: { fontSize: "22px", fontWeight: 700, color: "var(--maq-black)", marginBottom: "10px" },
    detailDesc: { fontSize: "14px", color: "var(--maq-gray-600)", lineHeight: 1.65, marginBottom: "16px" },
    ...
    railBtnActive: { border: "1px solid var(--maq-red)", borderLeftWidth: "3px", boxShadow: "0 1px 4px rgba(186,20,26,0.10)" },
    railIcon: {
        width: "36px",
        height: "36px",
        borderRadius: "8px",
...
export function DataAndAnalyticsCapabilities() {
    const s = useStyles();
        ...
        <section className={s.section} id="data-analytics-capabilities">
            <div className={s.inner}>
                <div className={s.head}>
                    ...
                    <p className={s.sub}>Capabilities that turn data into reliable, scalable analytics and real-time decisioning.</p>
                </div>
                <div className={s.panel}>
                    <div>
                        ...
                        <div className={s.tagRow}>{sel.tags.map((t) => <span key={t} className={s.tag}>{t}</span>)}</div>
                        <Button
                            appearance="outline"
                            className={s.knowMore}
                            as="a"
                            href={`mailto:customersuccess@maqsoftware.com?subject=${encodeURIComponent(sel.name + " - Data & Analytics")}`}
                        >
                            Know more
                        </Button>
                    </div>
