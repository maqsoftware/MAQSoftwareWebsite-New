...
const useStyles = makeStyles({
    ...
    title: { fontSize: "28px", fontWeight: 700, color: "var(--maq-black)", margin: "0 0 6px", letterSpacing: "-0.01em" },
    sub: { fontSize: "14px", color: "var(--maq-gray-600)", margin: 0 },
    grid: { display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "20px", "@media (max-width: 960px)": { gridTemplateColumns: "1fr" } },
    card: {
    ...
    cover: {
        height: "100px",
        background: "linear-gradient(135deg, var(--maq-red-pale) 0%, var(--maq-off-white) 60%, var(--maq-surface-cream) 100%)",
        borderBottom: "1px solid var(--maq-border)",
    },
    body: { padding: "20px", display: "flex", flexDirection: "column", gap: "10px", flex: 1 },
    cardTitle: { fontSize: "16px", fontWeight: 700, color: "var(--maq-black)", lineHeight: 1.35, margin: 0 },
...
export function DataAndAnalyticsInsights() {
    const s = useStyles();
    return (
                    ...
                    {insights.map((i) => (
                        <a key={i.title} className={s.card} href={i.href} target="_blank" rel="noreferrer">
                            ...
                            <div className={s.body}>
                                ...
                                <span className={s.read}>Read more <ArrowRight16Regular /></span>
                            </div>
                        </a>
                    ))}
                </div>
