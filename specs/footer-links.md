...
|---|---|---|
| Case studies | `/insights/case-studies` | Internal route |
| Power BI custom visuals | `/insights/power-bi-custom-visual-guide` | Internal route |
| Best practice guides | `/insights/best-practice-guides` | Internal route |
| Webinars | `null` | Render as non-interactive `<span>` |

> Webinars has no internal route in the current router. It must **not** point to `#` (which creates confusing scroll-to-top behaviour). Render as plain text until a route is added.
...
| Label | Route / URL | Notes |
|---|---|---|
| Who we are | `/who-we-are` | Internal route |
